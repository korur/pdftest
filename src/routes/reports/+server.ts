...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    doc.setFontSize(12);
    doc.text('Transliteration: "This is Japanese sample text. This will be converted to a PDF report."', 20, 110);
    
    // Add information about the text
    doc.setFontSize(12);
    doc.text('Original Japanese text: 日本語のサンプルテキストです。これはPDFレポートに変換されます。', 20, 130);
    doc.text('Note: Server-side PDF generation has limited support for non-Latin characters.', 20, 150);
